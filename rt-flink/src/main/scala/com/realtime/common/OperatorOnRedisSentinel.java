...
public class OperatorOnRedisSentinel {
    /**
     * 向缓存中设置字符串内容
     * @param key key
     * @param value value
     ...
     * @return
     */
    public static boolean set(Jedis jredis, int db, String key,JSONObject value, int seconds){
        try {
            jredis.select(db);
     ...
     * @return
     */
    public static boolean set(Jedis jredis, int db, String key, JSONArray value, int seconds){
        try {
            jredis.select(db);
            jredis.setex(key, seconds, value.toJSONString());
            return true;
     ...
     * @return
     */
    public static Set<String> keys(Jedis jredis, int db, String keys){
        try {
            jredis.select(db);
            Set<String> ss = jredis.keys(keys);
            return ss;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
